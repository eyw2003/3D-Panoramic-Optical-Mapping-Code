...
% Changes made in the following lines by Lea Melki on 03/29/2021 to avoid out of memory error when doing the phase movies 100 frames at a time (use a for loop one frame at a time instead of removing all zeros at once)
...
handles.MapTotal=MapTotal;
%
MapTotalBW = MapTotal(:,:,:,1);
MapTotalBW(MapTotalBW==0)=-1000; % added to replace the out of memory operation on entire 4D structure
...
% MapFinalCamA=handles.MapFinalCamA;
% MapFinalCamB=handles.MapFinalCamB;
% MapFinalCamC=handles.MapFinalCamB;
% MapFinalCamD=handles.MapFinalCamD;
% for i=1:size(MapFinalCamA,4)
%     temp=MapFinalCamA(:,:,:,i);
%     temp(temp==0)=-1000;
%     MapFinalCamA(:,:,:,i)=temp;
%     temp=MapFinalCamB(:,:,:,i);
...
%Comment the following 3 lines to not waste time and not save the raw .mat phase movie frames, just the time series format needed for Amira
if OverwriteIndex == 1
