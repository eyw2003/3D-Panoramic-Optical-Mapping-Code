function [MapFinalCamA,MapFinalCamB,MapFinalCamC,MapFinalCamD,MapTotal,MapRef] = ProjectionOptMapv3(SilhouetteTotal,threshold,AxX,AxY,AxZ,TSquare,SSquare,RSquare,QSquare,OptCamAFileName,Pathname)
...
BoundariesA=bwboundaries(VirtualAMap3);
for ii=1:size(BoundariesA{1,1},1)
    VirtualAMap3(BoundariesA{1,1}(ii,1),BoundariesA{1,1}(ii,2))=0;
end
figA = figure;
subplot(121)
imagesc(OptMapCamA)
caxis([0 max(OptMapCamA(:))])
...
BoundariesB=bwboundaries(VirtualBMap3);
for ii=1:size(BoundariesB{1,1},1)
    VirtualBMap3(BoundariesB{1,1}(ii,1),BoundariesB{1,1}(ii,2))=0;
end
figB = figure;
subplot(121)
imagesc(OptMapCamB)
caxis([0 max(OptMapCamB(:))])
title('Original activation map')
subplot(122)
...
BoundariesC=bwboundaries(VirtualCMap3);
for ii=1:size(BoundariesC{1,1},1)
    VirtualCMap3(BoundariesC{1,1}(ii,1),BoundariesC{1,1}(ii,2))=0;
end
figC = figure;
subplot(121)
imagesc(OptMapCamC)
caxis([0 max(OptMapCamC(:))])
title('Original activation map')
subplot(122)
...
BoundariesD=bwboundaries(VirtualDMap3);
for ii=1:size(BoundariesD{1,1},1)
    VirtualDMap3(BoundariesD{1,1}(ii,1),BoundariesD{1,1}(ii,2))=0;
end
figD = figure;
subplot(121)
imagesc(OptMapCamD)
caxis([0 max(OptMapCamD(:))])
title('Original activation map')
subplot(122)
...
%CamB
clear indOptMap CoordOptMap CoordOptMapRot
...
%SilhouetteTotal
clear indOptMap CoordOptMap CoordOptMapRot
indOptMap = find(SilhouetteTotal);
[CoordOptMap(:,1),CoordOptMap(:,2),CoordOptMap(:,3)] = ind2sub(size(Recon3DCamD),indOptMap);
CoordOptMapRot=transformPoint3d(CoordOptMap,ROTMatrix);
OptMapRot = zeros(size(Recon3DCamD));
for ii=1:length(indOptMap)
...
TransMatrixS = createTranslation3d(indXZero-ellFitS(1), indYZero-ellFitS(2),indZZero-ellFitS(3)); 
CoordOptMapFinal = transformPoint3d(CoordOptMapRot,TransMatrixS);
SilhouetteFinal = zeros(size(Recon3DCamD));
for ii=1:length(indOptMap)
    if round(CoordOptMapFinal(ii,1))<=0 || round(CoordOptMapFinal(ii,2))<=0 || round(CoordOptMapFinal(ii,3))<=0
        testCoordNeg = 0;
    else
        SilhouetteFinal(round(CoordOptMapFinal(ii,1)),round(CoordOptMapFinal(ii,2)),round(CoordOptMapFinal(ii,3))) = SilhouetteTotal(CoordOptMap(ii,1),CoordOptMap(ii,2),CoordOptMap(ii,3));
    end
end
...
title('Please select the symmetry axis of the section')
CorrectAxis = 0;
while CorrectAxis == 0
...
for ii = 1:size(MapRotatedCamA,3)
    ...
    SilhouetteFinal2(:,:,ii) = imrotate(SilhouetteFinal(:,:,ii),AngRot2,'nearest','crop');
    subplot(121)
    imagesc(MapRotatedCamA(:,:,ii))
    colormap(jet)
    caxis([-max(max(MapRotatedCamA(:,:,ii))) max(max(MapRotatedCamA(:,:,ii)))])
...
for ii = 1:size(MapRotatedCamA,3)
    ...
    for kk = 1:size(MapRotatedCamA,2)
       [indD,~] = find(MapRotatedCamD2(:,kk,ii),1,'last');
       [indB,~] = find(MapRotatedCamB2(:,kk,ii),1,'first');
       if kk == 1
           indBPrev = 0;
           indDPrev = 0;
       end
...
h = waitbar(0,'Projecting points common to cam A and B');
for ii = 1:length(indCompAB)
    ...
    waitbar(ii/length(indCompAB),h);
end
close(h)

h = waitbar(0,'Projecting points common to cam A and C');
for ii = 1:length(indCompAC)
    ...
    waitbar(ii/length(indCompAC),h);
end
close(h)

h = waitbar(0,'Projecting points common to cam A and D');
...
for ii = 1:length(indAOnly)
    ...
    waitbar(ii/length(indAOnly),h);
end
close(h)

h = waitbar(0,'Projecting points common to cam B and C');
for ii = 1:length(indCompBC)
    ...
    waitbar(ii/length(indCompBC),h);
end
close(h)

h = waitbar(0,'Projecting points common to cam B and D');
...
for ii = 1:length(indBOnly)
    ...
    waitbar(ii/length(indBOnly),h);
end
close(h)

h = waitbar(0,'Projecting points common to cam C and D');
