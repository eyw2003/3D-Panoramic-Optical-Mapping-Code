...
function [ProjectedMap,OptMapFMovie,R,ThetaMeshAxis,ZMeshAxis] = ProjectionOptMap(OptMap,SilhouetteFinal,AxX,AxY,AxZ)

nTheta = 180;
AxTheta = linspace(0,pi,nTheta);
dx = AxX(2)-AxX(1);
dy = AxY(2)-AxY(1);
h = waitbar(0,'Projecting the 3-D optical map onto the silhouette');
