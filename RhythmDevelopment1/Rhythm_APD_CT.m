...
% cgloschat@gmail.com or igorphase@wustl.edu
%
% Modification Log:
...
anal_select = uicontrol('Parent',anal_data,'Style','popupmenu','FontSize',12,'String',{'-----','Activation','Conduction','APD/CT','Phase','Dominant Frequency'},'Position',[5 85 165 25],'Callback',{@anal_select_callback});
...
savemap_button = uicontrol('Parent',anal_data,'Style','pushbutton','FontSize',12,'String','Save map','Position',[228 2 103 30],'Callback',{@savemap_button_callback});

% APD/CT specific buttons
...
remove_motion_click = uicontrol('Parent',anal_data,'Style','checkbox','FontSize',10,'String','Remove Mot.','Visible','on','Position',[230 35 100 25]);
% remove_motion_click_txt = uicontrol('Parent',anal_data,'Style','text','FontSize',1,'String','','Visible','on','Position',[248 15 50 25]);
calc_apd_button = uicontrol('Parent',anal_data,'Style','pushbutton','FontSize',12,'String','Regional APD/CT','Position',[125 2 103 30],'Callback',{@calc_apd_button_callback});
...
set([f,p1,filelist,selectdir,refreshdir,loadfile,movie_scrn,movie_slider, signal_scrn1,signal_scrn2,signal_scrn3,...
    ...
    starttimemap_edit,endtimemap_text,endtimemap_edit,createmap_button,savemap_button,minapd_text,minapd_edit,maxapd_text,maxapd_edit,...
    percentapd_text,percentapd_edit,remove_motion_click,calc_apd_button,expwave_button,...
...
set([removeBG_button,bg_thresh_edit,bg_thresh_label,perc_ex_edit,perc_ex_label,bin_button,filt_button,removeDrift_button,norm_button,...
    ...
    createmap_button,savemap_button,minapd_edit,minapd_text,maxapd_edit,maxapd_text,percentapd_edit,percentapd_text,remove_motion_click,...
...
set([invert_cmap,starttimemap_text,starttimemap_edit,endtimemap_text,...
    endtimemap_edit,createmap_button,savemap_button,minapd_text,minapd_edit,maxapd_text,...
    maxapd_edit,percentapd_text,percentapd_edit,remove_motion_click,...
    calc_apd_button],'Visible','off')
    ...
    function selectdir_callback(~,~)
        ...
        if dir_name ~= 0
            ...
            set([removeBG_button,bg_thresh_edit,bg_thresh_label,perc_ex_edit,...
                ...
                createmap_button,savemap_button,minapd_edit,minapd_text,maxapd_edit,maxapd_text,...
                percentapd_edit,percentapd_text,remove_motion_click,...
...
%% Data Analysis Selection
    function anal_select_callback(~,~)
        ...
        if anal_state == 1
            ...
            set(savemap_button,'Enable','off') 
        elseif anal_state == 2
            % Turn needed buttons on
            set([invert_cmap,starttimemap_text,starttimemap_edit,endtimemap_text,...
                endtimemap_edit,createmap_button,savemap_button],'Visible','on','Enable','on')
            % Turn unneeded buttons off
            set([minapd_text,minapd_edit,maxapd_text,maxapd_edit,percentapd_text,...
                percentapd_edit,remove_motion_click,...
        ...
        elseif anal_state == 4
            ...
            set([invert_cmap,starttimemap_text,starttimemap_edit,endtimemap_text,...
                endtimemap_edit,createmap_button,savemap_button,minapd_text,minapd_edit,maxapd_text,...
                maxapd_edit,percentapd_text,percentapd_edit,remove_motion_click,...
                calc_apd_button],'Visible','on','Enable','on')
        elseif anal_state == 5
            % Turn on create map button
            set(createmap_button,'Visible','on','Enable','on')
            set(savemap_button,'Visible','on','Enable','on')
...
%% Regional APD/CT Calculation
    function calc_apd_button_callback(~,~)
        % Read APD/CT Parameters
...
%% Button to create activation map
    function createmap_button_callback(~,~)
        ...
        elseif check == 3
            ...
            handles.CONDMap=condMap(handles.cmosData,handles.a_start,handles.a_end,handles.Fs,handles.bg,rect);
%             cMap(handles.cmosData,handles.a_start,handles.a_end,handles.Fs,handles.bg,rect);
        ...
        elseif check == 4
            ...
            handles.percentAPD = str2double(get(percentapd_edit,'String'));
            handles.APDMap = apdMap(handles.cmosData,handles.a_start,handles.a_end,handles.Fs,handles.percentAPD,handles.cmap);
            close(gg)
        % FOR PHASE MAP CALCULATION
        elseif check == 5
            handles.PhaseMapMovie = phaseMap(handles.cmosData,handles.starttime,handles.endtime,handles.Fs,handles.cmap);
        elseif check == 6
            gg=msgbox('Calculating Dominant Frequency Map...');
            handles.DomFreqMap = calDomFreq(handles.cmosData,handles.Fs,handles.cmap);
            close(gg)
        end
    end

%% Button to save maps
    function savemap_button_callback(~,~)
        % CHECK ANALYSIS MODE
        check = get(anal_select,'Value');
        
        % FOR ACTIVATION
        if check == 2
            PosCamName = strfind(handles.filename,'A');
            if isempty(PosCamName)
                PosCamName = strfind(handles.filename,'B');
                if isempty(PosCamName)
                    PosCamName = strfind(handles.filename,'C');
                    if isempty(PosCamName)
                        PosCamName = strfind(handles.filename,'D');
                    end
                end
            end
            FileNameSave = strcat(handles.dir,'/',handles.filename(1:PosCamName(end)),'ActivationMap.mat');
            Actmap = handles.actMap;
            save(FileNameSave,'Actmap')
        ...
        elseif check == 3
            ...
            FileNameSave = strcat(handles.dir,'/',handles.filename(1:PosCamName(end)),'ConductionMap.mat');
            CONDmap = handles.CONDMap;
            save(FileNameSave,'CONDmap')
        ...
        elseif check == 4
            ...
            FileNameSave = strcat(handles.dir,'/',handles.filename(1:PosCamName(end)),'APDMap.mat');
            APDmap = handles.APDMap;
            save(FileNameSave,'APDmap')
        ...
        elseif check == 5
            ...
            FileNameSave = strcat(handles.dir,'/',handles.filename(1:PosCamName(end)),'PhaseMapMovie.mat');
            PhaseMapMovie = handles.PhaseMapMovie;
            save(FileNameSave,'PhaseMapMovie')
            
        % FOR DOMINANT FREQ MAP CALCULATION
        elseif check == 6
            PosCamName = strfind(handles.filename,'A');
            if isempty(PosCamName)
                PosCamName = strfind(handles.filename,'B');
            ...
            FileNameSave = strcat(handles.dir,'/',handles.filename(1:PosCamName(end)),'DomFreqMap.mat');
            DomFreqMap = handles.DomFreqMap;
            save(FileNameSave,'DomFreqMap')
        end
    end
